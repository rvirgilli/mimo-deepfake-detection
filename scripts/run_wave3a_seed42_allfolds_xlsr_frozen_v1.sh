...
DOCROOT=docs/current/wave3a_seed42_allfolds_xlsr_frozen_v1
RUNROOT=experiments/runs/wave3a_xlsr_training_reference_seed42_allfolds_v1
mkdir -p "$DOCROOT"
FREE=$(nvidia-smi --query-gpu=memory.free --format=csv,noheader,nounits | head -1 || echo 0)
echo "GPU free MiB at launch: $FREE" >&2
for FOLD in "${FOLDS[@]}"; do
  echo "=== wave3a seed42 xlsr_frozen_backend fold ${FOLD} ===" >&2
...
folds=['CLAMTTS','GPST','MASKGCT','NS2','NS3','SIMPLESPEECH1','SIMPLESPEECH2','UNIAUDIO','VALLE']
docroot=Path('docs/current/wave3a_seed42_allfolds_xlsr_frozen_v1')
runroot=Path('experiments/runs/wave3a_xlsr_training_reference_seed42_allfolds_v1')
rows=[]
for fold in folds:
...
summary={
    'schema':'mimodf-wave3a-seed42-allfolds-summary/v1',
    'condition':'xlsr_frozen_backend',
    'seed':42,
    ...
    'caveats':['single seed directional diagnostic only','custom CoSG source-holdout split; not official CodecFake+ benchmark','no model-comparison or seed-general claim']
}
(docroot/'summary.json').write_text(json.dumps(summary, indent=2, sort_keys=True)+'\n')
lines=['# Wave 3A seed42 all-fold XLS-R frozen summary','','Condition: `xlsr_frozen_backend`','Seed: `42`','Checkpoint metric: `val_auroc`','','| Fold | Records | EER | AUROC | Bal acc | Bon recall | Spoof recall | Best epoch |','|---|---:|---:|---:|---:|---:|---:|---:|']
for r in rows:
