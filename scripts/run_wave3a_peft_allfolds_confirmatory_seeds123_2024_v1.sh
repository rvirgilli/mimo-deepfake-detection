...
RUN_ID="wave3a-peft-allfolds-confirmatory-seeds123-2024-v1"
START_TS=$(date +%s)
...
for seed in "${SEEDS[@]}"; do
  for fold in "${FOLDS[@]}"; do
    if ! run_fold "$seed" "$fold" 2; then
      ...
      echo "seed=$seed fold=$fold batch=2 failed; retry batch=1" >&2
      cp "$DOC_ROOT/seed_${seed}/${fold}.json" "$DOC_ROOT/seed_${seed}/${fold}.batch2_failed.json" 2>/dev/null || true
      run_fold "$seed" "$fold" 1 || exit $?
    fi
  done
done
summarize
